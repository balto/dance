...
class sfDate
{
	 ...
	 * @return	sfDate
	 */
	public static function getInstance($value = null, $timezone = null)
	{
		return new sfDate($value, $timezone);
	}
	...
	{
		$current_time_zone = Yii::app()->getTimeZone();

	    if (!is_null($timezone)) Yii::app()->setTimeZone($timezone);

	    $this->set($value);

	    if (!is_null($timezone)) Yii::app()->setTimeZone($current_time_zone);
	}
    ...
    public function locale_format($locale, $format)
    {
        $old_locale = setlocale(LC_ALL, '0');
        setlocale(LC_TIME, $locale);

        $result = strftime($format, $this->ts);

        setlocale(LC_ALL, $old_locale);
        return $result;
    }

	public function formatTimeZoneDateTime($format)
	{
	    Yii::app()->setTimeZone(Yii::app()->params["db_timezone"]);

	    $date = date($format, $this->ts);
	...
	{
		return date(Yii::app()->params["db_datetime_format"], $this->ts);
	}

	public function formatTimeZoneDbDateTime()
	{
	    Yii::app()->setTimeZone(Yii::app()->params["db_timezone"]);

	    $date = date(Yii::app()->params["db_datetime_format"], $this->ts);
	...
	public function formatTimeZoneDbDate()
	{
	    Yii::app()->setTimeZone(Yii::app()->params["db_timezone"]);

	    $date = date(Yii::app()->params["db_date_format"], $this->ts);
	...
	{
		return date(Yii::app()->params["db_date_format"], $this->ts);
	}

	public function formatDbTime()
	{
		return date(Yii::app()->params["db_time_format"], $this->ts);
	}
	...
	public function retrieve($unit = sfTime::DAY)
	{
		switch ($unit)
		{
		    ...
		    case sfTime::MONTH_NO_ZERO:
				return date('n', $this->ts);
			case sfTime::QUARTER:
