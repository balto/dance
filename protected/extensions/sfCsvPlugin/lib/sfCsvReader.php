...
class sfCsvReader
{
        ...
        if ($line = fgetcsv($this->file, $this->length, $this->delimiter, $this->enclosure))
        {
          ...
          if ($y > 0)
          {
            ...
            { // More columns in selection than in file or less common columns than selection
              throw new Exception("Túl sok, vagy túl kevés oszlop a CSV fájlban ($x oszlop helyett a fájlban $y van).");
            }
  ...
  public function read()
  {
    ...
    if ($line = fgetcsv($this->file, $this->length, $this->delimiter, $this->enclosure))
    {
      if (is_null($line[0])) return $this->read();    // Norbi: doksi alapján ez egy üres sor
      ...
      if (is_array($this->header['map']))
      {
        ...
        {
          if (array_key_exists($this->header['map'][$name], $line)) {
            if ($this->to !== null)
            {
