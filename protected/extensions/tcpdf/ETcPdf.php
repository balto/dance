...
class ETcPdf
{
   ...
   private $myTCPDF = null;
	 ...
	 */
   public function __construct($orientation, $unit, $format, $unicode, $encoding, $tcpdf_class = 'TCPDF')
   {
      ...
      if (!is_bool($unicode))
		...
		$this->myTCPDF = new $tcpdf_class($orientation, $unit, $format, $unicode, $encoding);
    ...
    */
	...
	{
		if (is_object($this->myTCPDF) && ($this->myTCPDF instanceof TCPDF)) return call_user_func_array(array($this->myTCPDF, $method), $params);
		else throw new CException(Yii::t('ETcPdf', 'Can not call a method of a non existent object'));
	}

	public function __set($name, $value)
	{
	   if (is_object($this->myTCPDF) && ($this->myTCPDF instanceof TCPDF)) $this->myTCPDF->$name = $value;
	   else throw new CException(Yii::t('ETcPdf', 'Can not set a property of a non existent object'));
	}

	public function __get($name)
	{
	   if (is_object($this->myTCPDF) && ($this->myTCPDF instanceof TCPDF)) return $this->myTCPDF->$name;
