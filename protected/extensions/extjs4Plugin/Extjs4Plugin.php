...
/**
 * @plugin           Extjs4Plugin
 * @description      Extjs4Plugin is a symfony plugin that provides an easy to use wrapper for the Ext javascript library
 ...
 *                    - Replaced all call_user_func and Extjs4Plugin:: by self::
...
class Extjs4Plugin {
  ...
  /**
   * Creates an instance of Extjs4Plugin.
   *
   * Usage:
   *
   *   $Extjs4Plugin = new Extjs4Plugin(
   *                           array
   *                           (
   *                             'adapter' => 'jquery', // config.extjs4_adapters
   *                             'theme'   => 'gray'    // config.extjs4_themes
   *                           ),
   *                           array
   *                           (
  ...
  private function setOptions($options)
  {
    ...
    {
      $this->options[$optName] = isset($options[$optName]) && array_key_exists($options[$optName], extjs4Config::get('extjs4_'.$optName.'s', array())) ? $options[$optName] : extjs4Config::get('extjs4_default_'.$optName);
    }
  }
   ...
   * config.extjs4_classes then Extjs4.class.constructor will rendered.
   *
   * Usage:
   *
   *   $Extjs4Plugin = new Extjs4Plugin(
   ...
   * @return string Javascript source of Extjs4.class
   */
  public function __call ($class, $attributes)
  {
    $classes = extjs4Config::get('extjs4_classes');
    if (is_array($classes) && isset($classes[$class]))
    {
      $object = extjs4Config::get($classes[$class]);
      return self::getExtObject($object['class'], $attributes[0]);
    }
  }

  /**
   * Creates Javascript source for Extjs4.class
   *
   * Usage:
   *
   *   Syntax A = short form without any options
   *   $Extjs4Plugin->Object(array
   *   (
   *     'id'       => 'id',
   *     'renderTo' => $Extjs4Plugin->asVar('document.body'),
   *     'items'    => array
   *     (
   *       $Extjs4Plugin->Object(array('title'=>'Object A')),
   *       $Extjs4Plugin->Object(array('title'=>'Object B'))
   *     )
   *   ));
   *
  ...
  public static function getExtObject($class, $params = array())
  {
    ...
    foreach (extjs4Config::get('extjs4_list_attributes') as $attribute)
    {
      if (isset($attributes[$attribute]) && !$attributes[$attribute] instanceof Extjs4Var)
      {
    ...
    $source = self::getExtObjectComponent($attributes, extjs4Config::get($class), $parameters, $datas);
  ...
  public function load()
  {
    ...
    $adapters = extjs4Config::get('extjs4_adapters', array());

    if ($this->options['adapter']) {
        foreach ($adapters[$this->options['adapter']] as $file)
        {
            $cs->registerScriptFile(Yii::app()->getBaseUrl() . extjs4Config::get('extjs4_js_dir') . $file, CClientScript::POS_HEAD);
        }
    }

    // add javascript sources for ext all
    //$debug = false ? '-debug' : ''; // if in web_debug mode, also use debug-extjs source

	//$cs->registerScriptFile(Yii::app()->getBaseUrl() . extjs4Config::get('extjs4_js_dir') . "ext-all$debug.js", CClientScript::POS_HEAD);

    if (isset($this->addons['js']))
    {
      foreach ($this->addons['js'] as $jsAddon)
      {
        $cs->registerScriptFile(Yii::app()->getBaseUrl() . extjs4Config::get('extjs4_js_dir') . "$jsAddon", CClientScript::POS_HEAD);
      }
    }

    // add css sources for ext all
    $cs->registerCssFile(Yii::app()->getBaseUrl() . extjs4Config::get('extjs4_css_dir') . 'ext-all.css');

    // add css sources for ext fixes
    //$response->addStylesheet(extjs4Config::get('extjs4_plugin_dir').'patches/fixes.css', 'first');

    // add css sources for theme
    $themes = extjs4Config::get('extjs4_themes', array());

    if ($this->options['theme']) {
        foreach ($themes[$this->options['theme']] as $file)
        {
            $cs->registerCssFile(Yii::app()->getBaseUrl() . extjs4Config::get('extjs4_css_dir') . $file);
        }
    }
    ...
    if (isset($this->addons['css']))
    {
      ...
      {
        $cs->registerCssFile(Yii::app()->getBaseUrl() . extjs4Config::get('extjs4_css_dir') . $cssAddon);
      }
    }
  }
   ...
   * @return string source
   */
  public function begin($web_root_url = '', $script = true)
  {
    $source  = self::LBR;
    if($script) $source .= sprintf("<script type='text/javascript'>%s", self::LBR);
    $source .= self::_comment(sprintf("%s// Extjs4Helper: %s%s", self::LBR, extjs4Config::get('extjs4_version'), self::LBR));
    $source .= sprintf("Ext.BLANK_IMAGE_URL = '%s'%s", $web_root_url.extjs4Config::get('extjs4_spacer'), self::LBR_SM);
   ...
   * Usage:
   *
   *    $Extjs4Plugin->initApplication('App');
   *
   *    => Ext.onReady(App.init, App);
   *
   *    $Extjs4Plugin->initApplication('App', true, 'myInit');
   *
   ...
   * Usage:
   *
   *     $Extjs4Plugin->customClass('Ext.app.symfony.ModuleA', array('title' => 'Module A', 'closable' => false));
   *
   ...
   * Usage:
   *
   *     $Extjs4Plugin->asAnonymousClass(array('name'=>'id','mapping'=>'id','type'=>'int'));
   *
   ...
   * Usage:
   *
   *     $Extjs4Plugin->asListener(array
   *     (
   *      'rowcontextmenu' => $Extjs4Plugin->asMethod(array
   *      (
   ...
   *
   * @param string var
   * @return Extjs4Var var
   */
  public static function asVar($var)
  {
    return new Extjs4Var($var);
  }
   ...
   *
   *    Syntax A = short form without any options
   *    $Extjs4Plugin->asMethod('alert("foo");');
   *
   *     => function() { alert("foo"); }
   *
   *    Syntax B = short form with parameters
   *    $Extjs4Plugin->asMethod(array('parameters' => 'msg', 'source' => 'alert(msg)'));
   *
  ...
  private static function _quote($key, $value, $always_quote = false)
  {
    if (is_array($value))
    {
      ...
      if (in_array($key, extjs4Config::get('extjs4_list_attributes')) && ($key!==0)) //don't know why 0 is a match
      {
    ...
    //always quote has of course an exception for numerics and Extjs4Vars
    if (!is_numeric($value) && (($always_quote && !$value instanceof Extjs4Var) || (!$value instanceof Extjs4Var && self::_quote_except($key, $value))))
    {
      $attribute = '\''.$value.'\'';
      return $attribute;
    }
  ...
  {
    $quoteExcept = extjs4Config::get('extjs4_quote_except');
    ...
    $listAttributes = extjs4Config::get('extjs4_list_attributes');
  ...
  {
    if (extjs4Config::get('extjs4_comment'))
    {
      return $comment;
    }
    else
...
/**
 * @class            Extjs4Var
 * @description      Extjs4Var is used by quoting logic which ignores variables of this class
